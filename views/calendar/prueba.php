...
$primerDiaSemana = new DateTime('monday this week');

// Obtener el último día de la semana (domingo)
$ultimoDiaSemana = new DateTime('sunday this week');
...
foreach ($sesiones as $sesion) {
    // Convertir las fechas de inicio y fin de la sesión en objetos DateTime
    $fechaHoraInicioSesion = new DateTime($sesion['fecha_hora_inicio']);
    $fechaHoraFinSesion = new DateTime($sesion['fecha_hora_fin']);
var_dump($fechaHoraInicioSesion) ;
var_dump($fechaHoraFinSesion) ;

    // Verificar si la sesión ocurre durante la semana actual
    if ($fechaHoraInicioSesion >= $primerDiaSemana && $fechaHoraFinSesion <= $ultimoDiaSemana) {
        // La sesión ocurre durante la semana actual
        echo "si.";
    } else {
        // La sesión no ocurre durante la semana actual
        echo "no.";
    }
}
...
<body>
    ...
    <button><a href="calendar.php">semana anterior</a></button>
</body>
